...
k = 10 # kcal/mol/Å^2 ( 1 kcal/mol/Å^2 = 418.4 kJ/mol/nm^2 )
exp = 1 
...
for ts in u.trajectory:
    ...
    for r1 in sel1.resids:
        ...
        if abs(sel1_1.positions[0][2]) > 72.0 :
            dz = abs(abs(sel1_1.positions[0][2])-72.0)
            fts = fts + ( k * ( dz ** exp ) )
            ftot = ftot + ( k * ( dz ** exp ) )
        elif abs(sel1_1.positions[0][2]) < 24.0 :
            dz = abs(abs(sel1_1.positions[0][2])-24.0)
            fts = fts + ( k * ( dz ** exp ) )
            ftot = ftot + ( k * ( dz ** exp ) )
    for r2 in sel2.resids:
        sel2_1 = sel2.select_atoms('resid %s'%(r2))
        if abs(sel2_1.positions[0][2]) > 72.0 :
            dz = abs(abs(sel2_1.positions[0][2])-72.0)
            fts = fts + ( k * ( dz ** exp ) )
            ftot = ftot + ( k * ( dz ** exp ) )
        elif abs(sel2_1.positions[0][2]) < 24.0 :
            dz = abs(abs(sel2_1.positions[0][2])-24.0)
            fts = fts + ( k * ( dz ** exp ) )
            ftot = ftot + ( k * ( dz ** exp ) )
    time = ts.frame / 1000
    force = fts / 2 / area    
